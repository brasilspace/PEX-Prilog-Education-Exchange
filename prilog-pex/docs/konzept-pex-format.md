...
├── tracks          [] id, label, scope: "class" | "subject", levels[]
...
Das vollständige Schema liegt als `pex.schema.json` bei (`$id: https://prilog.chat/schemas/pex/v2`). Jede PEX-Datei trägt `meta.format: "pex"` und `meta.schema`, damit ein Loader sie ohne Dateinamen erkennt.
