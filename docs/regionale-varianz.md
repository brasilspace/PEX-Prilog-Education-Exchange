...
- Die Overlays `de-hh`, `de-by`, `de-sh` und `ch-zh` (beigelegt) prüfen das Schema, nicht die Rechtslage. `de-sh` und `ch-zh` tragen in `notes`, welche Paragrafen noch gegen den Erlasstext zu lesen sind. Vor dem Einsatz an einer Schule gehört jede `rules`-Zeile gegen den aktuellen Verordnungstext gelesen.
