...
4. **Ausblenden** geht nur über `"disabled": true` am Element. Es gibt kein Löschen – so bleibt nachvollziehbar, dass Hamburg die Realschule *abgeschafft* hat und nicht *vergessen*. Verweise aus **Listen** (`grades[]`, `programs[]`, `tracks[]`, `qualifications[]`) auf ein ausgeblendetes Element bleiben gültig und werden beim Lesen gefiltert – eine Schule blendet die Berufslehre aus, ohne jeden Abschluss umzuschreiben, der sie nennt. **Anker** (`stage`, `domain`, `after_grade`, `scale`, `period`) dürfen dagegen nie auf ein ausgeblendetes Element zeigen. Regeln und Prüfungen folgen dem, wovon sie handeln: Zeigt `applies_to.to_program`, `exam.qualification` oder `at_grade` auf Ausgeblendetes, ist die Regel oder Prüfung **inaktiv** (Hinweis, kein Fehler) – wer das Gymnasium ausblendet, muss nicht jede kantonale Regel ausblenden, die es nennt.
