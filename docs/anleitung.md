...
4. **Ausblenden** geht nur über `"disabled": true` am Element. Es gibt kein Löschen – so bleibt nachvollziehbar, dass Hamburg die Realschule *abgeschafft* hat und nicht *vergessen*. Verweise aus **Listen** (`grades[]`, `programs[]`, `tracks[]`, `qualifications[]`) auf ein ausgeblendetes Element bleiben gültig und werden beim Lesen gefiltert – eine Schule blendet die Berufslehre aus, ohne jeden Abschluss umzuschreiben, der sie nennt. **Anker** (`stage`, `domain`, `after_grade`, `at_grade`, `from_grade`/`to_program` …, `scale`, `period`) dürfen dagegen nie auf ein ausgeblendetes Element zeigen.
