...
Skalare Felder (Strings, Zahlen, Booleans) – die höhere Schicht überschreibt.
Objekte ohne `id` (`terminology`, `calendar`, `grading` als Ganzes) – tiefer Merge; Schlüssel der höheren Schicht überschreiben einzeln, fehlende bleiben.
Listen mit `id` (`stages`, `grades`, `programs`, `tracks`, `qualifications`, `subjects`, `subject_domains`, `rules`, `exams`, `legal`, `scales`, `periods`) – Merge nach `id`, nicht Anhängen:
...
Ausblenden geht nur über `"disabled": true` am Element. Es gibt kein Löschen – so bleibt nachvollziehbar, dass Hamburg die Realschule abgeschafft hat und nicht vergessen.
Listen ohne `id` (`grades` an einem Fach, `aliases`, `levels`? – nein, `levels` haben `id`) – die höhere Schicht ersetzt die Liste komplett. Wer ein Fach um einen Jahrgang erweitern will, schreibt die ganze `grades`-Liste neu.
...
Basis wählen und lesen. Alles, was dort schon stimmt, wird nicht wiederholt.
`meta` schreiben: `kind: overlay`, `extends`, `region`, `source`.
Was ist anders? In dieser Reihenfolge prüfen: Programme (welche gibt es, welche nicht → `disabled`), Jahrgangsgrenzen (G8/G9, Grundschule 6), Züge, Abschlüsse und ihre Namen, Fächer (Namen als `aliases`, neue Fächer, andere Jahrgänge), Notenskala und Zeugnisrhythmus, Begriffe.
Regeln, Prüfungen, Recht – je eine Zeile mit `source`. Übertritt, Schulpflicht, Abschlussprüfung (intern/extern), Schulgesetz, Datenschutz, Aufsicht, Aufbewahrung.
Nur ändern, was sich ändert. Ein Overlay mit 300 Zeilen ist meist eine kopierte Basis.
Validieren: Datei gegen das Schema, dann den Stapel referenziell.
Versionieren und ins Repo `prilog-pex` mit Review.
...
Übertrittsberatung, Anmeldung	`rules` mit `kind: transition
